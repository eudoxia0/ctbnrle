...
// it calculate the queries using gibbs sampling

// Usage:
// no inputs to run: ./gibbsquery >output_file
int main(int argc, char **argv)
{
	...
	// set up the approximate inference engine using gibbs sampler 
	SamplingInf ginf;
    // number of burn in iterations
	int numofburniniter = 1000;
    // number of samples
    int numofsamples = 1000;
	Sampler *sampler;
	sampler = new GibbsSampler(&m, &evid, numofburniniter);
	ginf.SetSampler(sampler);
	ginf.SetProcess(&m);
	ginf.SetNumSample(numofsamples);

	// output the results
	cout << "the expected query answer is:" << endl;
	cout << expect_q1 <<  " " << expect_q2 << endl;
	cout << "the approximate inference answer using gibbs sampler to the query is:" << endl;
	cout << ginf.CalcQuery(query1) << " " << ginf.CalcQuery(query2) << endl;
	return 0;
}

...
// it calculate the queries using exact inference

// Usage:
// no inputs to run: ./exactquery >output_file
int main(int argc, char **argv) {
	...
	// set up the exact Markov inference engine 
	ExactMarkovInf einf;
	einf.SetProcess(&m);
	einf.SetTrajectory(&evid);

	// output the results
	cout << "the expected query answer is:" << endl;
	cout << expect_q1 <<  " " << expect_q2 << endl;
	cout << "the exact inference answer to the query is:" << endl;
	cout << einf.CalcQuery(query1) << " " << einf.CalcQuery(query2) << endl;
	return 0;
}
